...
local function addOrUpdateWorkspaceItem(workspaceName, monitorId, isSelected)
	...
	-- sbar.exec("sleep 1 && echo TEST", function(result, exit_code)
	-- 	print(result)
	-- end)
...
local function getWorkspacesForMonitor(monitorId, callback)
	safeExec(LIST_WORKSPACES:format(monitorId), function(workspacesOutput)
		local workspaces = {}
		local priorityOrder = { y = 1, u = 2, i = 3, o = 4, p = 5 }
		...
		table.sort(workspaces, function(a, b)
			local aPriority = priorityOrder[a] or math.huge
			local bPriority = priorityOrder[b] or math.huge
